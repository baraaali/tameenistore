...
|
*/
Route::get('get-sub-cat-list' ,'ServicePanelController@getSubCategoryList')->name('get-sub-cat-list');

Route::get('get-mini-sub-cat-list' ,'ServicePanelController@getMiniSubCategoryList')->name('get-mini-sub-cat-list');
//////////////
...
Route::get('dashboard/get-user-notifications','UserNotificationController@getUserNotifications')->name('get-user-notifications');
...
Route::post('/ads/notify/{type?}','SearchController@userNotify')->name('user_notify');
...
Route::get('/view/models/{id}/{name}/{model_id}/{shape_id}/{passengers}',function($brand_id,$name,$model_id,$shape_id,$passengers){
    ...
    if($model_id != 0) array_push($conditions,['id','=',$model_id]);
    if($shape_id != 0) array_push($conditions,['care_shape_id','=',$shape_id]);
    if($passengers != 0) array_push($conditions,['passengers','=',$passengers]);
   // return $conditions;
    return \App\models::where($conditions)
    ->whereNotIn('id',$ids)->get();
});
Route::get('/view/childerns/models/{brand_id}/{doc_id}/{model_id}/{shape_id}/{passengers}',
function($brand_id,$doc_id,$model_id,$shape_id,$passengers){
    $user_id=auth()->user()->id;
    $ids=\App\InsuranceDocument::where(['user_id'=>$user_id,'other_id'=>$doc_id])->pluck('model_id')->toArray();
    //return $ids;
    $conditions = [];
    if($brand_id != 0){
        array_push($conditions,['brand_id','=',$brand_id]);
...
Route::get('/view/childerns/models/search/{brand_id}/{doc_id}/{model_id}/{shape_id}/{passengers}',
function($brand_id,$doc_id,$model_id,$shape_id,$passengers){
    $user_id=auth()->user()->id;
    $rows=\App\InsuranceDocument::where(['user_id'=>auth()->user()->id,'other_id'=>$doc_id]);

    if($brand_id != 0) $rows->where('brand_id',$brand_id);
    if($model_id != 0) $rows->where('model_id',$model_id);
    if($shape_id != 0)
    $rows->whereHas('idmodel',function($q) use ($shape_id){
    $q->where('care_shape_id',$shape_id);
    });
    if($passengers != 0)
    $rows->whereHas('idmodel',function($q) use ($passengers){
    ...
    $lang =  App::getlocale();
    $view = strval(view('dashboard.insurance-user.single-selected-brands',compact('rows','lang')));
    $count = count($rows);
...
Route::get('/view/goverment/{id}',function($id){
    return DB::table('countries')->where('parent',$id)->get();
});
...
// mcenter services
Route::get('/cp/mcenter-services/{lang?}','ControlPanelController@mcenterServices')->name('mcenterServices');
...
//additional-services
Route::get('/cp/mcenter-additional-services/{lang?}','ControlPanelController@mcenterAdditionalServices')->name('mcenterAdditionalServices');;
...
Route::post('/cp/mcenter-requests','ControlPanelController@mcenterRequests')->name('cp-mcenter-requests');
...
Route::delete('/cp/mcenter-requests/{id}','ControlPanelController@MaintenanceRequest')->name('delete-mcenter-requests');
...
// account info
Route::get('/dashboard/user/account-info' , 'SecurityController@accountInfo')->name('account-info');
Route::post('/dashboard/user/account-info' , 'SecurityController@accountInfo')->name('account-info');
...
Route::get('/{lang?}','FrontEndController@home')->name('home');
Route::get('/set-country/{country_id}/{lang?}','FrontEndController@setCountry')->name('set-country');

/* commercial_ads */
Route::get('/commercial-ad/{id}/{lang?}','FrontEndController@showCommercialAd')->name('show-commercial-ad');
Route::get('/commercial-ads/all/{category_id?}/{lang?}','FrontEndController@allCommercialAds')->name('all-commercial-ads');

/* agencies sales */
Route::get('/agencies/sales-agencies/{lang?}','FrontEndController@salesAgencies')->name('sales-agencies');
Route::get('/agencies/sales-agencies/view/{id}/{lang?}','FrontEndController@singleSalesAgencies')->name('single-sales-agencies');

/* agencies sales */
Route::get('/agencies/rental-agencies/{lang?}','FrontEndController@rentalAgencies')->name('rental-agencies');
Route::get('/agencies/rental-agencies/view/{id}/{lang?}','FrontEndController@singleRentalAgencies')->name('single-rental-agencies');

/* vehicles */
Route::get('/vehicles/sell/{vehicle_id}/{lang?}','FrontEndController@vehiclesSell')->name('vehicles-sell');
Route::get('/vehicles/sell/view/{vehicle_id}/{lang?}','FrontEndController@viewVehicleSell')->name('vehicles-sell-view');

Route::get('/vehicles/rent/{vehicle_id}/{lang?}','FrontEndController@vehiclesRent')->name('vehicles-rent');
Route::get('/vehicles/rent/view/{vehicle_id}/{lang?}','FrontEndController@viewVehicleRent')->name('vehicles-rent-view');

Route::post('/vehicles/search','FrontEndController@searchvehicles')->name('search-vehicles');

/* services */
Route::get('/services/{cat_id}/{sub_cat}/{lang?}','FrontEndController@services')->name('services');
Route::get('/services-single/items/{id}/{lang?}','FrontEndController@serviceSingle')->name('services-single');
Route::post('/services/search','FrontEndController@searchServices')->name('search-services');


/* services vehicles */
Route::get('/services-vehicles/{cat_id}/{lang?}','FrontEndController@servicesVehicles')->name('services-vehicles');
Route::post('/services-vehicles/mcenters/search','FrontEndController@servicesVehicles')->name('services-mcenters-search');
Route::get('/services-vehicles/mcenters/profil/{id}/{lang?}','FrontEndController@mcenterProfil')->name('services-vehicles-mcenter-profil');

/*  set likes */
Route::post('/actions/like','FrontEndController@setLike')->name('actions-set-like');

/*  global search */
Route::post('/search/keyword','FrontEndController@searchByKeyWord')->name('search-by-key-word');


/*  set insurance */
Route::get('/insurance/single','FrontEndController@insuranceSingle')->name('insurance-single');
Route::get('/insurance/complete','FrontEndController@insuranceComplete')->name('insurance-complete');

/* static pages */
Route::get('/page/{page}','FrontEndController@getStaticPage')->name('get-static');
...
Route::post('/dashboard/renew-memebership-mcenter','DashboardController@renewMemebershipMcenter')->name('renew-memebership-mcenter');
 ...
 */
///banner user
Route::get('/dashboard/user/banners/{lang?}','BannerUserController@banners')->name('all_banners');
Route::post('/dashboard/user/edit-banner/{id}','BannerUserController@editBanner')->name('edit-user-banner');
Route::post('/dashboard/user/renew_banner/{id}','BannerUserController@renewBanner')->name('renew_banner');
Route::get('/dashboard/user/banners/get-info/{id}/{lang?}','BannerUserController@getInfo');
Route::post('/dashboard/user/banners/add-user-banner','BannerUserController@addUserBanner')->name('add-user-banner');
Route::get('/dashboard/user/banners/delete-banner/{id}','BannerUserController@deleteBanner')->name('deleteBanner');

//advertisements
Route::get('/dashboard/advertisements' , 'AdvertisementController@showItem')->name('advertisements-show');
//Route::resource('/dashboard/advertisements','modulesController');
Route::post('/dashboard/advertisements' , 'AdvertisementController@showItem');
Route::get('/dashboard/advertisements/changeStatus' , 'AdvertisementController@itemChangeStatus')->name('advertisementsStatus');
Route::get('/dashboard/advertisements/show' , 'AdvertisementController@itemshow')->name('dash.items.show');
Route::post('/dashboard/advertisements/renew_date_item' , 'AdvertisementController@renewItem')->name('advertisements.renew');
Route::get('/dashboard/advertisements/delete/{id}' , 'AdvertisementController@delete')->name('advertisements.destroy');
...
Route::post('/dashboard/mcenters/rate','MaintenanceRequestController@rate')->name('rate-menters');
...
Route::post('/dashboard/cars/index','CarsController@index')->name('cars-search');
...
Route::resource('dashboard/get/banners','BannerController');
...
Route::get('/view/mcenters/profil/{id}/{lang?}','SearchController@SearchController')->name('mcenters-profil');
...
Route::get('/dashboard/insurance/{lang?}','ControlPanelController@insuranceSingle')->name('dashboard-insurance');
Route::get('/dashboard/complete/insurance/{lang?}','ControlPanelController@insuranceComplete')->name('all_complete');
...
Route::get('cp/get_all_brand/order-by-brand/{brand_id}/{model_id}/{name}/{passengers}/{shape}','ControlPanelController@getAllBrandsByAjax')->name('get_all_brands_by_ajax');  //
...
Route::get('/cp/renew_tammeen/{id}/{type}/{lang?}','ControlPanelController@renewTammeen')->name('renew_tammeen');
Route::get('/cp/select-new-membership','ControlPanelController@selectNewMembership')->name('selectNewMembership');  //
Route::delete('/cp/delete-model-from-completeDoc/{name}/{model_id}','ControlPanelController@deleteModelFromCompleteDoc')->name('delete-model-from-completeDoc');  //
...
Route::delete('cp/inDocument/delete-single-doc/{id}','ControlPanelController@deleteSingleDoc')->name('delete-model-from-single');
...
Route::get('/hidden_request/{id}','InsuranceController@hiddenRequest')->name('hidden_request')->middleware('auth');
Route::post('/submitCompleteDoc/changeStatus','InsuranceController@submitCompleteDocChangeStaus')->name('submitCompleteDocChangeStatus')->middleware('auth');


//website moduke
Route::get('shboard/da/website','WebsiteController@edit')->name('edit-website');
...
Route::post('myfatoorah/pay','FatooraController@payMoney')->name('payMoney');  //
Route::get('myfatoorah/callBack','FatooraController@callBack')->name('callBack_url');  //
Route::get('myfatoorah/error','FatooraController@error')->name('error_url');  //
...
//notify
Route::get('/dashboard/notify/{type?}' , 'NotifyController@index')->name('admin_notify');
Route::get('/dashboard/notify/watched/{id}' , 'NotifyController@watchAds')->name('notify.eye');
Route::get('/dashboard/notify/delete/{id}/{type?}' , 'NotifyController@delete')->name('notify.delete');

//admin items
Route::get('/dashboard/items' , 'ItemsController@index')->name('dash.items');
Route::get('/dashboard/items/create' , 'ItemsController@create')->name('items-create');
Route::post('/dashboard/items' , 'ItemsController@index');
Route::get('/dashboard/items/itemChangeStatus' , 'ItemsController@itemChangeStatus')->name('itemChangeStatus');
Route::get('/dashboard/items/show' , 'ItemsController@itemshow')->name('dash.items.show');
Route::post('/dashboard/items/renew_date_item' , 'ItemsController@renewItem')->name('dashboard_renew_date_item');

//panel items // accessories
Route::get('/dashboard/items/index','ItemsController@index');
Route::get('/dashboard/items/view','ItemsController@create')->name('items-view');
Route::post('/dashboard/items/store','ItemsController@store')->name('items-Store');
Route::post('/dashboard/items/update','ItemsController@update')->name('items-Update');
Route::get('/dashboard/items/delete/{id}','ItemsController@destroy')->name('items-delete');
Route::post('/dashboard/items/renew','ItemsController@renewAds')->name('renew_ads_member_from_balance');
...
Route::get('/dashboard/member-insurance/delete/{id}' , 'MemberInsuranceController@delete')->name('memberInsurance.delete');
...
/////////////////////cats
Route::get('/dashboard/cats/index','CatController@index')->name('cats');
Route::post('/dashboard/cats/store','CatController@store')->name('cats-store');
Route::post('/dashboard/cats/update','CatController@update')->name('cats-update');
Route::get('/dashboard/cats/delete/{id}','CatController@destroy')->name('cats-delete');
Route::get('/dashboard/cats/restore/{id}','CatController@restore')->name('cats-restore');
Route::get('/dashboard/cats/archive','CatController@archive')->name('cats-archive');
Route::get('/dashboard/cats/catChangeStatus','CatController@catChangeStatus')->name('catChangeStatus');
/////////////////////sub cats
Route::get('/dashboard/sub_cats/index','SubCatController@index')->name('sub_cats');
Route::post('/dashboard/sub_cats/store','SubCatController@store')->name('sub_cats-store');
Route::post('/dashboard/sub_cats/update','SubCatController@update')->name('sub_cats-update');
Route::get('/dashboard/sub_cats/delete/{id}','SubCatController@destroy')->name('sub_cats-delete');
Route::get('/dashboard/sub_cats/restore/{id}','SubCatController@restore')->name('sub_cats-restore');
Route::get('/dashboard/sub_cats/archive','SubCatController@archive')->name('sub_cats-archive');
Route::get('/dashboard/sub_cats/catChangeStatus','SubCatController@catChangeStatus')->name('catChangeStatus');
/////////////////////mini sub cats
Route::get('/dashboard/mini_sub_cats/index','MiniSubCatController@index')->name('mini_sub_cats');
Route::post('/dashboard/mini_sub_cats/store','MiniSubCatController@store')->name('mini_sub_cats-store');
Route::post('/dashboard/mini_sub_cats/update','MiniSubCatController@update')->name('mini_sub_cats-update');
Route::get('/dashboard/mini_sub_cats/delete/{id}','MiniSubCatController@destroy')->name('mini_sub_cats-delete');
Route::get('/dashboard/mini_sub_cats/restore/{id}','MiniSubCatController@restore')->name('mini_sub_cats-restore');
Route::get('/dashboard/mini_sub_cats/archive','MiniSubCatController@archive')->name('mini_sub_cats-archive');
// Route::get('/dashboard/sub_cats/catChangeStatus','SubCatController@catChangeStatus')->name('catChangeStatus');

//payment_report
Route::get('/dashboard/payment_report_user/{lang?}','FatooraController@paymentReport')->name('payment_report_user');  //
Route::get('/dashboard/payment_report' , 'AdminPaymentController@index')->name('admin.report.index');
Route::post('/dashboard/report_filter/' , 'AdminPaymentController@showItem')->name('admin.report');


//new modul

//dep new
Route::resource('/dashboard/new-service-memberships' , 'NewServiceMembershipController')->except('show');
Route::get('/dashboard/new-service-memberships/{id}' , 'NewServiceMembershipController@delete')->name('new-service-memberships.delete');

Route::get('/cp/servies/{lang?}','ServicePanelController@allService');
Route::get('/cp/servies/addServices/{lang?}','ServicePanelController@addServices')->name('addServices');
Route::post('/cp/servies/storeServices/{lang?}','ServicePanelController@storeServices')->name('storeServices');
Route::get('/cp/servies/editServices/{id}/{lang?}/{type?}','ServicePanelController@editServices')->name('editServices');
Route::post('/cp/servies/updateServices/{id}/{lang?}','ServicePanelController@updateServices')->name('updateServices');
Route::get('/cp/servies/deleteServices/{id}/{lang?}','ServicePanelController@deleteServices')->name('deleteServices');
Route::post('/cp/servies/renewServices/{lang?}','ServicePanelController@renewServices')->name('renewServices');

Route::get('/cp/all-servies/{lang?}','ServicePanelController@allServices')->name('new_services');

Route::post('/cp/all-servies/{lang?}','ServicePanelController@allServices')->name('search_services');

Route::get('/cp/service-details/{id}/{lang?}','ServicePanelController@ServiceDetails')->name('ServiceDetails');

//  ads modules
Route::get('/dashboard/modules' , 'modulesController@showItem')->name('modules-show');
Route::post('/dashboard/modules' , 'modulesController@showItem')->name('modules-search');
Route::get('/dashboard/modules/create' , 'modulesController@create')->name('modules-create');
Route::get('/dashboard/modules/edit/{id}' , 'modulesController@edit')->name('modules-edit');
Route::post('/dashboard/modules/update/{id}' , 'modulesController@update')->name('modules-update');
Route::post('/dashboard/modules/store' , 'modulesController@store')->name('modules-store');
//Route::resource('/dashboard/modules','modulesController');
Route::get('/dashboard/modules/changeStatus' , 'modulesController@itemChangeStatus')->name('moduleChangeStatus');
Route::get('/dashboard/modules/show' , 'modulesController@itemshow')->name('dash.items.show');
Route::post('/dashboard/modules/renew_date_item' , 'modulesController@renewItem')->name('modules_renew_date_item');
Route::get('/dashboard/modules/delete/{id}' , 'modulesController@delete')->name('modules.destroy');
Route::post('/dashboard/modules/sub-category-list' , 'modulesController@getSubCategoryList')->name('modules.sub-category-list');
Route::post('/dashboard/modules/mini-category-list' , 'modulesController@getMiniSubCategoryList')->name('modules.mini-sub-category-list');

//

Route::get('/insurance/requests/{lang?}','InsuranceController@insurancerequestsuser')->name('insurancerequestsuser');
Route::get('/complete/requests/{lang?}','InsuranceController@complete')->name('insurancerequestscomplete');
