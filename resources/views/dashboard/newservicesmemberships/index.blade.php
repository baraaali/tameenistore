...
@section('content')
    @php $arr=['عاديه','فضيه','مميزة','ذهبيه'];
    @endphp
...
{{--                            </a>--}}
                            <a class=" btn btn-danger" onclick="return confirm('Are you sure?')" href="{{ route('new-service-memberships.destroy', $price->id)}}">
    ...
    <div class="modal fade" id="create" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form class="form" method="post" action="{{route('new-service-memberships.store')}}">
                    @csrf
        ...
        <div class="modal fade" id="edit{{$price->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="tdocument">
                <div class="modal-content">
                    ...
                    <form class="form" method="post" action="{{route('new-service-memberships.update',$price->id)}}">
                        @csrf
                        {{ method_field('put') }}
                        ...
                        <div class="modal-body">
                            ...
                            <select name="type" required class="SpecificInput form-group form-control">
                                @for($i=0;$i<4;$i++)
                                    <option value="{{$i}}" @if($i==$price->type) selected @endif>{{$arr[$i]}}</option>
                                @endfor
                            </select>
                        </div>
