...
<div class="modal fade " id="{{isset($service_edit)? "edit-modal" : "add-new" }}">
    <div class="modal-dialog" role="document">
        <form action="{{route('create-additional-mcenter-service')}}" method="post">
            @csrf
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                              <label for="mcenter_service_id">{{__('site.follow to service')}}</label>
                              <select required class="form-control select2" name="mcenter_service_id" id="mcenter_service_id">
                                  @foreach ($mcenter_services as $service)
                                  <option @if(isset($service_edit) && $service_edit->service->id == $service->id) selected @endif value="{{$service->id}}" >{{$service->getName()}}</option>
                                  @endforeach
                              </select>
                            </div>
                        </div>
