...
<div class="card text-white bg-primary shadow">
    ...
    top: 6px;">
	  إضافة قسم الفرع فرعي   
		</h5>
		<a href="{{route('service_child_categories.index')}}" class="btn btn-light"  style="float: left" >
	...
	<div class="card-body" style="background-color: white;color:#31353D">
		 @if ($errors->any())
                  ...
                  @endif
		<form class="form" action="{{route('service_child_categories.store')}}" method="POST" enctype="multipart/form-data">
			@csrf
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select value="" class="form-control select2" name="service_sub_category_id" id="service_sub_category_id"> 
			...
			<div  class="row new-service_child_category position-relative base">
					<div class="col-md-6">
						<div class="form-group">
							<label>
								الإسم بالعربي
...
@section('js')
    <script>
        ...
        {
            $('.new-service_child_category').find('.preview').attr('src','')
            $('.new-service_child_category').find('.preview').hide()
            $('.new-service_child_category').find('[name="en_name[]"]').val('')
            $('.new-service_child_category').find('[name="ar_name[]"]').val('')
            $('.new-service_child_category').find('[name="ar_description[]"]').val('')
            $('.new-service_child_category').find('[name="en_description[]"]').val('')
            $('.new-service_child_category').find('[name="image[]"]').val('')
        }
        $(document).ready(function(){
            $('.new').on('click',function(){
            $base =  $('.new-service_child_category').clone()[0]
            $($base).removeClass('new-service_child_category')
            $('.new-service_child_category').before($base)
            clearElement();
            ...
            $('.new-service_child_category').removeClass('new-service_child_category')
            $($base).addClass('new-service_child_category')
         })
          $(document).on('click','.remove',function(){
              $(this).parent().remove()
              if(!$('.new-service_child_category').length)
              $('form > .base').addClass('new-service_child_category')
          })
          $('.save').on('click',function(){
            
             var $en_names = $('[name="en_name[]"]')
             var $ar_names = $('[name="ar_name[]"]')
             var $service_sub_category_id = $('[name="service_sub_category_id"]')
             var errors = {};
              for (let i = 0; i < $en_names.length; i++) {
                ...
                if(!$service_sub_category_id.val())
                {
                 $($service_sub_category_id).parent().after('<div class="invalid-feedback d-block my-4">هذا الحقل ضروري</div>')
                errors['service_sub_category_id'+i] = true
                }else{
                    $($service_sub_category_id).parent().find('.invalid-feedback').remove()
                    delete errors['service_sub_category_id'+i] 
                }
                }
