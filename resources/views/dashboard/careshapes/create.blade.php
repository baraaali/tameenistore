...
<div class="card text-white bg-primary shadow">
	...
	إضافة شكل
		...
		<a href="{{route('careshapes.index')}}" class="btn btn-light"  style="float: left" >
	...
	<div class="card-body" style="background-color: white;color:#31353D">
		 @if ($errors->any())
                  ...
                  @endif
		<form class="form" action="{{route('careshapes.store')}}" method="POST" enctype="multipart/form-data">
			@csrf
			...
			<div  class="row new-careshape position-relative base">
					<div class="col-md-3">
...
@section('js')
    <script>
        ...
        var saveData = function()
        {
            ...
            //             console.log(res);
            //             // if(res === true)
            //             // window.location.href = "{{route('careshapes.index')}}";
			// 		}
        ...
        {
            $('.new-careshape').find('.preview').attr('src','')
            $('.new-careshape').find('.preview').hide()
            $('.new-careshape').find('[name="en_name[]"]').val('')
            $('.new-careshape').find('[name="ar_name[]"]').val('')
            $('.new-careshape').find('[name="image[]"]').val('')
        }
        $(document).ready(function(){
            $('.new').on('click',function(){
            $base =  $('.new-careshape').clone()[0]
            $($base).removeClass('new-careshape')
            $('.new-careshape').before($base)
            clearElement();
            ...
            $('.new-careshape').removeClass('new-careshape')
            $($base).addClass('new-careshape')
         })
          $(document).on('click','.remove',function(){
              $(this).parent().remove()
              if(!$('.new-careshape').length)
              $('form > .base').addClass('new-careshape')
          })
